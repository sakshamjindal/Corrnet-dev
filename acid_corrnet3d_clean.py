...
class LitCorrNet3D(pl.LightningModule):
    ...
    def __init__(
        ...
        ls_coeff: list = [10.0, 0.01, 0.01, 0],
    ...
    def _run_loss(self, txyz1, txyz2, p, out_a, out_b, label): 
        xyz1=txyz1
        xyz2=txyz2
        ...
        # supervised_loss = F.cross_entropy(p.transpose(1, 2), torch.max(label.transpose(1, 2), dim=-1).indices)
        # L2 loss
        # supervised_loss= F.cross_entropy(p, torch.max(label.transpose(1, 2), dim=-1).indices)
        supervised_loss = torch.mean(self._batch_frobenius_norm(p, label.transpose(1, 2)))

        return rec_term, rank_term, mfd_term, supervised_loss
        
    def step(self, batch, batch_idx):
        label, pinput1, input2, index_ = batch
        
        p, out_a, out_b =self._run_step(pinput1,input2)
        rec_term, rank_term, mfd_term, supervised_loss = self._run_loss(pinput1.transpose(2, 1), input2.transpose(2, 1), p, out_a, out_b, label.transpose(2, 1))
        loss = self.rec_coeff*rec_term + self.rank_coeff*rank_term + self.mfd_coeff*mfd_term + self.sl_coeff*supervised_loss
        logs = {
            "rec_loss": rec_term,
            "rank_loss": rank_term,
            "mfd_loss": mfd_term,
    ...
    def training_step(self, batch, batch_idx):
        ...
        self.log(name = 'train_loss',value=loss.item(), on_step=True, on_epoch=True, prog_bar=False)
        # log the loss of the last step
        # for k, v in logs.items():
        #     self.log(name = k,value=v.item(), on_step=True, on_epoch=False, prog_bar=False)
    ...
    def validation_step(self, val_batch, batch_idx):
        ...
        # self.log(name = 'val_loss',value=loss.item(), on_step=True, on_epoch=False, prog_bar=False)
        # # log the loss of the last step
        # # for k, v in logs.items():
        # #     self.log(name = k,value=v.item(), on_step=True, on_epoch=False, prog_bar=False)
    ...
    def add_model_specific_args(parent_parser):
        ...
        parser.add_argument("--pair_count", type=int, default=0)
        parser.add_argument("--object_count", type=int, default=0)
        return parser
...
def cli_main(args=None):
    ...
    logger = pl.loggers.TensorBoardLogger("tb_logs_debug", name=args.exp_name)
    trainer = pl.Trainer.from_argparse_args(args, gpus=str(args.gpus), benchmark=True, deterministic=True, logger=logger, max_epochs=5000) #gpu
